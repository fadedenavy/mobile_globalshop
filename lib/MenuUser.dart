...
import 'package:mobile_globalshop/views/DetailProdukLaris.dart';
...
import 'package:mobile_globalshop/views/History.dart';
...
class _MenuUserState extends State<MenuUser> {
  ...
  Future<void> _lihatData() async {
    ...
    Future<void> _lihatLaris() async {
    listlaris.clear();
    setState(() {
      larisLoad = true;
    });
    final response = await http.get(BaseUrl.urlDataTerlaris);
    if (response.contentLength == 2) {
    } else {
      final data = jsonDecode(response.body);
      data.forEach((api) {
        final ab = new ProdukTerlarisModel(
            api['id_barang'],
            api['nama_barang'],
            api['harga'],
            api['image']);
        ...
        child: ListView(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder: (context) => new History()));
              },
            ),
            ListTile(
              leading: Material(
  ...
  Widget buildLaris(ProdukTerlarisModel listlaris) {
    return GestureDetector(
      onTap: () {
        //halaman detail
        Navigator.push(
